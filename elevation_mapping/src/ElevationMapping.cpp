...
void ElevationMapping::setupServices()
{
  ...
  ros::AdvertiseServiceOptions advertiseServiceOptionsForGetFusedSubmap =
    ros::AdvertiseServiceOptions::create<grid_map_msgs::GetGridMap>(
  ...
  ros::AdvertiseServiceOptions advertiseServiceOptionsForGetRawSubmap =
    ros::AdvertiseServiceOptions::create<grid_map_msgs::GetGridMap>(
  ...
  disableUpdatesService_ =
    nodeHandle_.advertiseService("disable_updates", &ElevationMapping::disableUpdatesServiceCallback, this);
  ...
  reloadParametersService_ =
    nodeHandle_.advertiseService("reload_parameters", &ElevationMapping::reloadParametersServiceCallback, this);
}
...
void ElevationMapping::setupTimers()
{
  ...
  mapUpdateTimer_ =
    nodeHandle_.createTimer(parameters.maxNoUpdateDuration_, &ElevationMapping::mapUpdateTimerCallback, this, true, false);

  if (!parameters.fusedMapPublishTimerDuration_.isZero())
  {
    ros::TimerOptions timerOptions = ros::TimerOptions(
      parameters.fusedMapPublishTimerDuration_, [this](auto&& PH1) { publishFusedMapCallback(PH1); }, &fusionServiceQueue_, false,
      false);
  ...
  {
    ros::TimerOptions timerOptions = ros::TimerOptions(
      parameters.visibilityCleanupTimerDuration_, [this](auto&& PH1) { visibilityCleanupCallback(PH1); },
...
bool ElevationMapping::readParameters(bool reload)
{
  ...
  if (fusedMapPublishingRate == 0.0)
  {
    ...
    ROS_WARN("Rate for publishing the fused map is zero. The fused elevation map will not be published unless the service "
             "`triggerFusion` is "
             "called.");
  }
  ...
  SensorProcessorBase::GeneralParameters generalSensorProcessorConfig{
    nodeHandle_.param("robot_base_frame_id", std::string("/robot")), parameters.mapFrameId_
  };
...
void ElevationMapping::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& pointCloudMsg,
                                          ...
                                          const SensorProcessorBase::Ptr& sensorProcessor_)
{
  ...
  if (!receivedFirstMatchingPointcloudAndPose_)
  {
    ...
    {
      ROS_WARN_THROTTLE(
        5, "No corresponding point cloud and pose are found. Waiting for first match. (Warning message is throttled, 5s.)");
      return;
    }
    else
    {
...
void ElevationMapping::mapUpdateTimerCallback(const ros::TimerEvent& /*unused*/)
{
  ...
  if ((lastPointCloudUpdateTime_ - time) <= parameters.maxNoUpdateDuration_)
...
bool ElevationMapping::updatePrediction(const ros::Time& time)
{
  ...
  {
    ROS_ERROR("Requested update with time stamp %f, but time of last update was %f.", time.toSec(),
                                                     ...
                                                     grid_map_msgs::GetGridMap::Response& response)
{
  ...
  ROS_DEBUG("Elevation submap request: Position x=%f, y=%f, Length x=%f, y=%f.", requestedSubmapPosition.x(),
...
bool ElevationMapping::disableUpdatesServiceCallback(std_srvs::Empty::Request& /*request*/,
                                                     std_srvs::Empty::Response& /*response*/)
{
  ROS_INFO("Disabling updates.");
  auto [parameters, parameterGuard]{ parameters_.getDataToWrite() };
  parameters.updatesEnabled_ = false;
  ros::ServiceClient client = nodeHandle_.serviceClient<std_srvs::Empty>("/unitree_ctrl/stop_map_update");
  std_srvs::Empty srv;
  client.call(srv);
  return true;
}

bool ElevationMapping::enableUpdatesServiceCallback(std_srvs::Empty::Request& /*request*/,
                                                    std_srvs::Empty::Response& /*response*/)
{
  ROS_INFO("Enabling updates.");
  auto [parameters, parameterGuard]{ parameters_.getDataToWrite() };
  parameters.updatesEnabled_ = true;
  ros::ServiceClient client = nodeHandle_.serviceClient<std_srvs::Empty>("/unitree_ctrl/start_map_update");
  std_srvs::Empty srv;
  client.call(srv);
  return true;
}
...
bool ElevationMapping::initializeElevationMap()
{
  ...
  if (parameters.initializeElevationMap_)
  {
        ...
        elevation_mapping::InitializationMethods::PlanarFloorInitializer)
    {
      ...
      try
      {
        transformListener_.waitForTransform(parameters.mapFrameId_, parameters.targetFrameInitSubmap_, ros::Time(0),
                                            ros::Duration(5.0));
                                                    ...
                                                    grid_map_msgs::SetGridMap::Response& /*response*/)
{
       ...
       sourceLayerIterator++)
  {
    ...
    if (map_.getRawGridMap().exists(*sourceLayerIterator))
    {
      ...
      for (grid_map::GridMapIterator destinationIterator(map_.getRawGridMap()); !destinationIterator.isPastEnd();
           ++destinationIterator)
      {
                                              ...
                                              grid_map_msgs::ProcessFile::Response& response)
{
  ...
  response.success =
    static_cast<unsigned char>(grid_map::GridMapRosConverter::saveToBag(map_.getFusedGridMap(), request.file_path, topic));
...
bool ElevationMapping::reloadParametersServiceCallback(std_srvs::Trigger::Request& /*request*/,
...
void ElevationMapping::stopMapUpdateTimer() { mapUpdateTimer_.stop(); }
